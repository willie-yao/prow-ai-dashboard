...
func TestService_AgenticDisabled_UsesCuratorMode(t *testing.T) {
	...
	s.Analyze(context.Background(), &http.Client{}, newRun("j", "1"), tc)
...
func TestService_ToolsUnsupported_FallsBackOnce(t *testing.T) {
	...
	s.Analyze(context.Background(), &http.Client{}, newRun("j", "1"), tc1)
	tc2 := newFailedTC("Test B", "msg-b")
	s.Analyze(context.Background(), &http.Client{}, newRun("j", "1"), tc2)
