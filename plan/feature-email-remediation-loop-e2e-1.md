...
| TASK-008 | Repeat `initial-failure` using a newly constructed pipeline and unchanged data. Assert no additional email, no second fix PR, no additional marker search after the tracked reference is loaded, and unchanged notification/remediation emailed indexes. | ✅ | 2026-07-21 |
...
| TASK-014 | Decode `remediations.json` after presubmit, verified, and recurrence steps. Assert public status and links are present, while source commit, head SHA from observations, exact test identity, error hash, failed match strings, and `IneligibleCommits` remain absent. | ✅ | 2026-07-21 |
