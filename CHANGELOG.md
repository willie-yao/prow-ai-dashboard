...
- New `ai.pattern_analysis` (**on by default**) adds a job-level, cross-build
  ...
  top of the job page. Cheap and self-gating, so it is left on; set
  `pattern_analysis: false` to disable it on a tightly rate-limited endpoint.
  See [docs/agentic.md](docs/agentic.md#pattern-analysis).
