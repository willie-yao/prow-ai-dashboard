...
| Prompt drafting times out on a slow provider | Source retrieval plus the structured extraction and revision stages exceeded the onboarding-specific budget. | Retry with `--prompt-timeout 30m` or another reviewed value. The fetcher timeout and project `ai.timeout` do not change onboarding drafting. |
