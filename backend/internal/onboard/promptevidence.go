...
const promptEvidenceRevisionInstruction = `Revise one validated prompt evidence object against the quality rubric and return the complete structured object. The original repository excerpts are intentionally not supplied. You may remove exact duplicates or reorganize existing items, but factual strings, keyed evidence fields, and source references must remain unchanged. You may not introduce new factual claims, source references, artifact paths, repositories, failure patterns, transient classes, or investigation capabilities. Keep unknown details unresolved. Do not introduce portal, SSH, shell, browser, local CLI, or live-cluster investigation as observed evidence.`
