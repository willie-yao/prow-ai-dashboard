...
func Run(ctx context.Context, opts Options) error {
	...
	case modeK8s:
		...
		if files["CHECKLIST.md"], err = render(k8sChecklistTmpl, checklistData{
			Name: data.Name, DashboardOwner: dashOwner, DashboardName: dashName, EngineRef: data.EngineRef, Namespace: data.Namespace,
		}); err != nil {
			return err
		}
	default:
...
func scaffoldPRBody(name, mode string) string {
	...
	return fmt.Sprintf("Generated by `prow-ai-dashboard onboard` for **%s**. The job sweep confirmed discovery finds jobs.\n\n"+
		...
		"- %s\n\n"+
		"See `CHECKLIST.md` in this PR for the full steps.", name, finalStep)
}
