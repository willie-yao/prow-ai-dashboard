...
|---|---|---|
| On-disk analysis cache (keyed by mode+hash) | The Task name fingerprints project/build/test identity plus the failure prompt, artifact-tree seed hash, evidence-plan hash, provider/model, timeout/retry, and Tool definitions. Re-applying it is a no-op, so the K8s object store is the cache. `-version` remains a manual override for external semantic changes. | `AnalysisTaskName` / `AnalysisContractHash`; `Apply` is idempotent |
