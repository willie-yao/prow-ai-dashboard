...
|---|---|---|
| On-disk analysis cache (keyed by mode+hash) | The Task name fingerprints project/build/test identity plus the failure prompt, artifact-tree seed hash, evidence-plan hash, provider/model, timeout/retry, and Tool definitions. Re-applying it is a no-op, so the K8s object store is the cache. Ingested analyses persist the exact Task name and contract hash before they are considered current. `-version` remains a manual override for external semantic changes. | `AnalysisTaskName` / `AnalysisContractHash`; `Apply` is idempotent |
...
| Initial failure evidence | The producer includes bounded JUnit failure message/body text, a filtered artifact path tree, and a matched recipe checklist with ranked exact artifact candidates. Per-build listings have a 10-second timeout and share a 45-second run-wide budget. | `FailurePrompt`, `ArtifactTreeSeed`, `EvidencePlanPrompt` |
